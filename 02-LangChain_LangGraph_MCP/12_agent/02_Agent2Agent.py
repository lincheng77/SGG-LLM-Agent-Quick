...
if __name__ == "__main__":
    try:
        ...
        ctrip_chain = create_ctrip_agent(model)
        meituan_chain = create_meituan_agent(model)
        didi_chain = create_didi_agent(model)
        print("✅ 所有Agent初始化完成！\n" + "="*90 + "\n")

        # 初始化A2A总协调Agent
        print("🔧 初始化A2A总协调Agent（调度核心）...")
        coor_chain = create_travel_coordinator_agent(model, ctrip_chain, meituan_chain, didi_chain)
        print("✅ 总协调Agent初始化完成！\n" + "="*90 + "\n")
